...
{
    public static WinnerControllerScript winnerControllerScript { get; set; }
    ...
    void Start()
    {
        ...
        if (!isGameScene)
        {
            ...
            if (GameManager.Instance.iWon)
            {
                ...
                audioSources[0].Play();
               // GameManager.Instance.playfabManager.addCoinsRequest(GameManager.Instance.payoutCoins *18/10);
               if (GameManager.Instance.isLocalMultiplayer == false)// && GameManager.Instance.isPlayingWithComputer==false)
               {  
                   // GameManager.Instance.coinsCount += firstPlacePrize;
                  // GameManager.Instance.playfabManager.apiManager.AddCoins(GameManager.Instance.payoutCoins);
               }
            }
            ...
            Debug.Log(prizeCoins);
            // prizeCoins = GameManager.Instance.payoutCoins *18/10f;
