...
public class ControlAvatars : MonoBehaviour
{
    ...
    void Start()
    {
        Debug.Log("Etststststtstststs Avater   " + gameObject.name);
    ...
    public void CancelWaitingForOnlineRoom()
    {
        Debug.LogError("1");
        if (PhotonNetwork.inRoom) { PhotonNetwork.LeaveRoom(); Debug.LogError("2"); }
        if (PhotonNetwork.insideLobby) { PhotonNetwork.LeaveLobby(); Debug.LogError("3"); }
    }
    ...
    public void PlayerJoined(int index, string id)
    {
        ...
        if (GameManager.Instance.opponentsIDs.Contains(id))
        {
            ...
            catch { Debug.LogError("Catch was called"); }
            ...
            Debug.Log("Required players count: " + GameManager.Instance.requiredPlayers);
            ...
            if (foundPlayer)
            {
                ...
                if (OpponentAvatar != null && OpponentAvatar.GetComponent<RectTransform>().anchoredPosition.y <= 0)
                {
                    ...
                    else
                    {
                        ...
                        StartCoroutine(countDownCoins(int.Parse(GameManager.Instance.payoutCoins.ToString())));
                    }
